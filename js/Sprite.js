...
class Sprite {
    constructor({ position, imageSrc, scale = 1, maxFrames = 1, holdFrames = 30, offsetFrame = { x: 0, y: 0 } }) {
        ...
        this.elapsedFrames = 0;         // How many frames have passed. Used for smoother animation.
        this.holdFrames = holdFrames;   // How many frames to wait until update the image with another frame. Used for smoother animation.
        this.offsetFrame = offsetFrame;
        this.frameWidth = this.image.width / this.maxFrames;  // Calculate frame width once for efficiency
    }
    ...
    draw() {
        c.drawImage(
            this.image,
            this.currentFrame * this.frameWidth, 0, this.frameWidth,
            this.image.height,
            this.position.x - this.offsetFrame.x, this.position.y - this.offsetFrame.y,
            this.frameWidth * this.scale, this.image.height * this.scale
        );
    }
...
export const background = new Sprite({
    position: { x: 0, y: 0 },
    imageSrc: '/assets/img/background.jpg'
});
